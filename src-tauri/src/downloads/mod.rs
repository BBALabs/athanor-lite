...
/// Blocking install-if-missing, used by the dev self-tests (debug builds
/// only); the evented `start` is the interactive route.
...
mod tests {
    ...
    /// smallest catalog artifact (llama-3.2-3b, ~2.0 GB), cancels mid-flight,
    ...
    fn download_resume_verify_real() {
        let spec = resolve_spec("llama-3.2-3b-instruct", "Q4_K_M").expect("catalog spec");
